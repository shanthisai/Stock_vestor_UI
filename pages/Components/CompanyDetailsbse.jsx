...
class CompanyDetailsbse extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      companyDetails: [],
  ...
  getCompanyDetails = async (company) => {
    await axios.get("/api/companydetailsbse?company=" + company).then((s) => {
      if (s.status === 200) {
  ...
  getStockDetails = async(company) => {
    await axios
      .get("/api/previousdaystockdetailsbse?company=" + company)
      .then((s) => {
        if (s.status === 200) {
  ...
  getSuggestion = async(company) => {
    await axios
        .get("/api/getsuggestionsbse?company=" + company)
        .then((t) => {
          console.log(t.data);
          if (t.status === 200) {
      ...
      <React.Fragment>
        ...
        {this.state.selectedCompany !== "" &&
          this.state.stockdetails.length !== 0 && (
            <Dashboardbse company={this.state.selectedCompany} />
          )}
      </React.Fragment>
    );
  }
...
export default withStyles(styles, { withTheme: true })(CompanyDetailsbse);
